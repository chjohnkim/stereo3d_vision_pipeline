...
class disparity:
    def __init__(self, method, data_path, DEVICE='cuda'):
        ...
        if self.method=='RAFT': 
            ...
            parser.add_argument('--restore_ckpt', help="restore checkpoint", default='RAFT_Stereo/models/raftstereo-middlebury.pth')
    ...
    def generate_folder(self):
        output_directory = Path(os.path.join(self.data_path, 'output', 'DM_'+self.method))
        ...
        if output_directory.exists():
            ...
            expected_num_files = len(glob.glob(os.path.join(self.data_path, 'output', 'LEFT_RECT/*')))
        ...
        left_images = sorted(glob.glob(os.path.join(self.data_path, 'output', 'LEFT_RECT/*'), recursive=True))
        right_images = sorted(glob.glob(os.path.join(self.data_path, 'output', 'RIGHT_RECT/*'), recursive=True))
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            cv2.imwrite(output_directory / "disparity{:04d}.png".format(numbers[-1]), disparity_map)
