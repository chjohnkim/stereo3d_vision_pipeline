...
def rectify_folder(data_path, camera_param_left, camera_param_right):
    ...
    left_rect_path = Path(os.path.join(data_path, 'output', 'LEFT_RECT'))
    right_rect_path = Path(os.path.join(data_path, 'output', 'RIGHT_RECT'))
    left_rect_path.mkdir(parents=True, exist_ok=True)
    right_rect_path.mkdir(parents=True, exist_ok=True)

    img_paths_L = []
    for root, _, files in os.walk(os.path.join(data_path, 'LEFT')):
        for file in files:
            if file.endswith('.jpg'):                
                img_paths_L.append(os.path.join(root, file))

    img_paths_R = []
    for root, _, files in os.walk(os.path.join(data_path, 'RIGHT')):
        for file in files:
            if file.endswith('.jpg'):                
                img_paths_R.append(os.path.join(root, file))

    assert len(img_paths_L) == len(img_paths_R), "Number of images in LEFT and RIGHT folders do not match"
    
    left_rect_num_files = len([file for file in left_rect_path.iterdir() if file.is_file()])
    right_rect_num_files = len([file for file in right_rect_path.iterdir() if file.is_file()])
    if left_rect_num_files == len(img_paths_L) and right_rect_num_files == len(img_paths_R):
        print("Rectified images already exist in output folder")
        return

    # Check if the output folder already contains rectified images
    for img_L_path, img_R_path in tqdm(list(zip(img_paths_L, img_paths_R))):
        ...
        cv2.imwrite(img_L_path.replace('.jpg', '_rect.jpg').replace('/LEFT', '/output/LEFT_RECT'), cv2.cvtColor(img_L_rect, cv2.COLOR_RGB2BGR))
        cv2.imwrite(img_R_path.replace('.jpg', '_rect.jpg').replace('/RIGHT', '/output/RIGHT_RECT'), cv2.cvtColor(img_R_rect, cv2.COLOR_RGB2BGR))    