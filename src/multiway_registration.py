...
class multiway_registration:
    def __init__(self, data_path, method, voxel_size, filter_nb_points, filter_radius, normal_radius, normal_max_nn, apply_filter):
    ...
    def _load_pointclouds_from_folder(self):
        pc_paths = sorted(glob.glob(os.path.join(self.data_path, 'output', 'PC_world_'+self.method, '*.ply'), recursive=True))
        dense_pointclouds = []
    ...
    def generate_pointclouds(self, out_name, out_voxel_size, out_filter_nb_points, out_filter_radius):
        ...
        out_path = Path(os.path.join(self.data_path, 'output', str(out_name)+'_'+self.method+'.ply'))
